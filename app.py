...
with st.sidebar:
    ...
    st.info("The system uses specialized agents for quality, statistics, ML, visualization and review.")
...
if uploaded:
    ...
    if sample.exists():
        ...
        st.info("Using the included sample weather dataset.")
    else:
...
if st.button("🚀 Analyze Dataset", type="primary", use_container_width=True):
    state = AnalysisState(user_request=request, dataframe=df)
...
if "result" in st.session_state:
    ...
    c1.metric("Rows", q.get("rows", 0))
    c2.metric("Columns", q.get("columns", 0))
    c3.metric("Missing Cells", q.get("missing_cells", 0))
    c4.metric("Duplicates", q.get("duplicate_rows", 0))
    ...
    if state.statistical_results:
        with st.expander("📈 Statistical Analysis", expanded=True):
            st.json(state.statistical_results)

    if state.ml_results:
        with st.expander("🤖 Machine Learning", expanded=True):
            st.json(state.ml_results)

    if state.visualization_results:
        with st.expander("📉 Visualizations", expanded=True):
            for fig_path in state.visualization_results.get("figures", []):
                st.image(fig_path, use_container_width=True)

    with st.expander("🛡️ Reviewer / Reliability", expanded=True):
        st.json(state.reviewer_results)
